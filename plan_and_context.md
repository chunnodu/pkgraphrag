...
| **3** | Mar 9–15 | 🛠 Build | Write Python parser: .mm XML → RDF triples (rdflib); run on first domain (e.g. Semantic web maps) | First domain loaded into Virtuoso/TopQuadrant | [ ] |
...
| **6** | Mar 30–Apr 5 | ⚙️ Integration | Generate embeddings for RDF node labels + literals; store in Chroma vector DB | Embeddings stored + searchable | [ ] |
...
## Technical Context & Findings (As of March 17, 2026)
...
*   **Live/MCP Agent Potential:** While the 12-week plan builds a batch pipeline to an external Triplestore (Virtuoso), the active Freeplane MCP server connection opens doors for secondary capabilities, such as an AI agent writing *back* into Freeplane directly.
